...
app = FastAPI(title="FraudGuard", version="1.3.0")
initialize_database()
...
def proxy_go(path:str,payload:dict|None=None,method:str="POST",headers:dict|None=None)->JSONResponse:
    ...
    try:
        with urllib.request.urlopen(req,timeout=12) as response:
            raw=response.read().decode("utf-8");data=json.loads(raw) if raw else {}
...
def create_transaction(transaction:TransactionCreate,request:Request):
    ...
    history_rows=get_user_transactions(transaction.user_id);history=[transaction_from_history(row) for row in history_rows]
