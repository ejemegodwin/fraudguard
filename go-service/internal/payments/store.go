...
func NewStore(path string) (*Store, error) {
	...
	data,err:=os.ReadFile(path); if errors.Is(err,os.ErrNotExist){return s,nil}; if err!=nil{return nil,fmt.Errorf("read payment store: %w",err)}; if len(data)==0{return s,nil}
	if err:=json.Unmarshal(data,&s.payments);err!=nil{return nil,fmt.Errorf("decode payment store: %w",err)};return s,nil
}
func(s *Store)Create(payment Payment)error{s.mu.Lock();defer s.mu.Unlock();if payment.TransactionID==""{return errors.New("transaction ID is required")};if _,ok:=s.payments[payment.TransactionID];ok{return fmt.Errorf("payment %s already exists",payment.TransactionID)};payment.UpdatedAt=time.Now().UTC();s.payments[payment.TransactionID]=payment;return s.persistLocked()}
